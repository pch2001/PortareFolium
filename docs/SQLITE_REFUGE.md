...
3. 로컬 admin도 일반 credentials 로그인으로 들어간다.
    ...
    refuge mode는 admin session을 자동 생성하지 않는다. `AUTH_SECRET` 또는 `NEXTAUTH_SECRET`이 없으면 Auth.js가 `/api/auth/session`에서 `MissingSecret`으로 실패하므로, 로컬 `.env.local`에 명시적으로 둔다.
