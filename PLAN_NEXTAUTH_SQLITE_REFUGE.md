...
- 공개 로그인 버튼은 유지 가능. 단 allowlist 이메일만 실제 로그인 허용

### Desired User Flow

1. 앱 업데이트
2. 오너가 기존 **Supabase 이메일/비밀번호**로 1회 로그인
3. 일반 admin 화면 대신 `/admin/migrate`로 강제 이동
4. 화면에서 Google OAuth 전환 가이드 확인
5. Google 로그인 완료
6. 완료 버튼으로 legacy Supabase 세션 해제
7. 이후부터는 NextAuth Google 로그인만 사용
